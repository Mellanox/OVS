...
static void
dump_flow_attr(const struct rte_flow_attr *attr, struct ds *s)
{
    ds_put_format(s,
...
static void
dump_flow_pattern(const struct rte_flow_item *item, struct ds *s)
{
...
static void
dump_flow_action(const struct rte_flow_action *actions, struct ds *s)
{
                                   ...
                                   struct flow_actions *actions)
{
    ...
    NL_ATTR_FOR_EACH_UNSAFE (ca, cleft, clone_actions, clone_actions_len) {
        ...
        if (clone_type == OVS_ACTION_ATTR_TUNNEL_PUSH) {
            ...
            action_items->raw_encap.data = (uint8_t *)tnl_push->header;
            action_items->raw_encap.preserve = NULL;
            action_items->raw_encap.size = tnl_push->header_len;

            add_flow_action(actions, RTE_FLOW_ACTION_TYPE_RAW_ENCAP,
                            &action_items->raw_encap);
        } else if (clone_type == OVS_ACTION_ATTR_OUTPUT) {
            bool last_action = (cleft <= NLA_ALIGN(ca->nla_len));

            if (!last_action) {
                return -1;
            }

            if (netdev_dpdk_flow_add_output_action(ca, info, action_items,
                                                   actions)) {
                return -1;
            }
        } else {
