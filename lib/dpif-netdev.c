...
dp_netdev_offload_init(void)
{
    ...
    if (ovsthread_once_start(&offload_thread_start)) {
        ...
        for (i = 0; i < netdev_offload_thread_nb(); i++) {
            mpsc_queue_init(&dp_offload_threads[i].offload_queue);
...
dp_netdev_append_ct_offload(struct ct_flow_offload_item *offload)
{
    ...
    mpsc_queue_insert(&dp_offload_threads[i].offload_queue, &offload_item->node);
    atomic_count_inc64(&dp_offload_threads[i].enqueued_offload);
    if (offload->op == DP_NETDEV_FLOW_OFFLOAD_OP_ADD) {
        atomic_count_inc64(&dp_offload_threads[i].enqueued_offload_add);
    }
}
                                ...
                                struct ovs_barrier *barrier)
{
    ...
    for (tid = 0; tid < netdev_offload_thread_nb(); tid++) {
        ...
        mpsc_queue_insert(&dp_offload_threads[tid].offload_queue, &item->node);
 ...
 * overflow_trcs = Amount of trace messages dropped due to queue overflow.
 * flow_add_msgs = Amount of flow add messages received by E2E cache.
 * flow_del_msgs = Amount of flow del messages received by E2E cache.
...
static struct e2e_cache_stats e2e_stats = {
    .generated_trcs = ATOMIC_COUNT_INIT(0),
    .processed_trcs = 0,
    .discarded_trcs = ATOMIC_COUNT_INIT(0),
    .aborted_trcs = ATOMIC_COUNT_INIT(0),
    .throttled_trcs = ATOMIC_COUNT_INIT(0),
    .queue_trcs = ATOMIC_COUNT_INIT(0),
    .overflow_trcs = ATOMIC_COUNT_INIT(0),
    .flow_add_msgs = ATOMIC_COUNT_INIT(0),
    .flow_del_msgs = ATOMIC_COUNT_INIT(0),
    .flush_flow_msgs = 0,
    .succ_merged_flows = 0,
    .merge_rej_flows = 0,
                        ...
                        struct dp_netdev *dp)
{
    ...
    dp_netdev_offload_flush_handle(&flush_item.item);
}
